# Captain's Log - TrueNAS Deployment Helper Script
# This script helps you deploy Captain's Log to your TrueNAS system
...
if ($Help) {
    Write-Host "Captain's Log - TrueNAS Deployment Helper"
    Write-Host ""
    Write-Host "Usage:"
...
Write-Host "Captain's Log - TrueNAS Deployment" -ForegroundColor Cyan
Write-Host "=========================================" -ForegroundColor Cyan
...
switch ($Method) {
    "1" {
        ...
        ssh root@$TruenasIP "mkdir -p /mnt/main_pool/captains-log"
        scp docker-compose.truenas.yml root@${TruenasIP}:/mnt/main_pool/captains-log/
        ...
        Write-Host "  cd /mnt/main_pool/captains-log" -ForegroundColor White
        ...
        Write-Host "  docker exec captains-log-backend npx prisma migrate deploy" -ForegroundColor White
        Write-Host ""
    ...
    "2" {
        ...
        ssh root@$TruenasIP "mkdir -p /mnt/main_pool/captains-log/backend && mkdir -p /mnt/main_pool/captains-log/frontend && mkdir -p /mnt/main_pool/captains-log/postgres && mkdir -p /mnt/main_pool/captains-log/nominatim && mkdir -p /mnt/main_pool/captains-log/uploads && mkdir -p /mnt/main_pool/captains-log/logs && chown -R 999:999 /mnt/main_pool/captains-log/postgres && chmod -R 755 /mnt/main_pool/captains-log"

        # Copy files
        Write-Host "Copying backend files..." -ForegroundColor Yellow
        scp -r backend root@${TruenasIP}:/mnt/main_pool/captains-log/

        Write-Host "Copying frontend files..." -ForegroundColor Yellow
        scp -r frontend root@${TruenasIP}:/mnt/main_pool/captains-log/
        ...
        Write-Host "  ssh root@$TruenasIP" -ForegroundColor White
        Write-Host "  cd /mnt/main_pool/captains-log" -ForegroundColor White
        Write-Host "  docker build -f backend/Dockerfile.prod -t captains-log-backend:latest ./backend" -ForegroundColor White
        Write-Host "  docker build -f frontend/Dockerfile.prod.truenas -t captains-log-frontend:latest ./frontend" -ForegroundColor White
        Write-Host "  docker-compose -f docker-compose.truenas.yml up -d" -ForegroundColor White
        Write-Host "  docker exec captains-log-backend npx prisma migrate deploy" -ForegroundColor White
        Write-Host ""
        Write-Host "Then access: http://${TruenasIP}:30600" -ForegroundColor Green
    ...
    "3" {
        ...
        docker push "$env:DOCKER_REGISTRY/captains-log-backend:latest"
        docker push "$env:DOCKER_REGISTRY/captains-log-frontend:latest"
