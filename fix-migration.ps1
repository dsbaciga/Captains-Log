...
$checkColumn = docker exec captains-log-backend sh -c 'psql "$DATABASE_URL" -t -c "SELECT column_name FROM information_schema.columns WHERE table_name=''users'' AND column_name=''timezone'';"'

if ([string]::IsNullOrWhiteSpace($checkColumn)) {
    Write-Host "Column does not exist - applying migration..." -ForegroundColor Yellow
    docker exec captains-log-backend sh -c 'psql "$DATABASE_URL" -c "ALTER TABLE users ADD COLUMN IF NOT EXISTS timezone VARCHAR(100) DEFAULT ''UTC'';"'
...
docker exec captains-log-backend npx prisma migrate resolve --rolled-back 20251015_add_user_timezone
