...
export default function ImmichBrowser({
    ...
    const loadThumbnails = async () => {
      ...
      for (const asset of assets) {
        ...
        try {
          ...
          if (!response.ok) {
            console.error(`[ImmichBrowser] Failed to fetch thumbnail ${asset.id}:`, response.status, response.statusText);
            continue;
          }
          ...
          console.error(`[ImmichBrowser] Error loading thumbnail ${asset.id}:`, error);
        }
      }
    };
  ...
  const loadData = async () => {
    ...
    try {
      ...
      if (view === 'albums') {
        ...
        console.log(`[ImmichBrowser] Loading assets by date range (page ${currentPage}):`, tripStartDate, 'to', tripEndDate);
        const result = await immichService.getAssetsByDateRange(tripStartDate, tripEndDate, { skip, take });
        console.log('[ImmichBrowser] Received assets:', result.assets?.length || 0, 'of', result.total);
        ...
        console.log(`[ImmichBrowser] Loading all assets (page ${currentPage}, no date filter)`);
      ...
      alert('Failed to load photos from Immich. Please check your Immich settings.');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  const allPageAssetsSelected = assets.length > 0 && assets.every(asset => selectedAssetIds.has(asset.id));
          ...
          {isLoading ? (
            ...
            <>
              ...
              {assets.length > 0 && (
                <div className="mb-4">
                  <button
                    onClick={allPageAssetsSelected ? handleDeselectAllOnPage : handleSelectAllOnPage}
                  ...
                  >
                    {allPageAssetsSelected ? 'Deselect All on Page' : 'Select All on Page'}
                  </button>
                </div>
              )}
                  ...
                  >
                    {blobUrl ? (
                      <img
                        ...
                        onError={(e) => console.error(`[ImmichBrowser] Image failed to load: ${asset.id}`, e)}
                      />
                    ) : (
          ...
          {!isLoading && view === 'assets' && totalAssets > ITEMS_PER_PAGE && (
            <div className="flex justify-center items-center gap-4 mt-6">
              ...
              <span className="text-sm text-gray-600 dark:text-gray-400">
                Page {currentPage} of {Math.ceil(totalAssets / ITEMS_PER_PAGE)} ({totalAssets} total photos)
              </span>
              <button
                onClick={() => setCurrentPage(p => Math.min(Math.ceil(totalAssets / ITEMS_PER_PAGE), p + 1))}
                disabled={currentPage >= Math.ceil(totalAssets / ITEMS_PER_PAGE)}
                className="px-4 py-2 bg-gray-200 dark:bg-gray-700 text-gray-700 dark:text-gray-300 hover:bg-gray-300 dark:hover:bg-gray-600 rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
              >
                Next
              </button>
        ...
        <div className="p-6 border-t border-gray-200 dark:border-gray-700 flex justify-between items-center">
          <p className="text-sm text-gray-600 dark:text-gray-400">
            {selectedAssetIds.size > 0
              ? `${selectedAssetIds.size} ${selectedAssetIds.size === 1 ? 'photo' : 'photos'} selected`
          ...
          <div className="flex gap-2">
            ...
            >
              Link {selectedAssetIds.size > 0 ? `${selectedAssetIds.size} ` : ''}Photo{selectedAssetIds.size !== 1 ? 's' : ''}
            </button>
          </div>
        </div>
      </div>
