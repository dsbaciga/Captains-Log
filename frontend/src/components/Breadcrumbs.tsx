...
export default function Breadcrumbs({ items, className = '' }: BreadcrumbsProps) {
  ...
  return (
    ...
    >
      <ol className="flex items-center text-sm font-body overflow-x-auto">
          ...
          return (
            <li key={index} className="flex items-center flex-shrink-0">
              ...
              {item.href && !isLast ? (
                <Link
                  to={item.href}
                  className="text-primary-600 dark:text-sky hover:text-primary-700 dark:hover:text-accent-400 hover:underline transition-colors whitespace-nowrap"
                >
                  {item.label}
                </Link>
              ) : (
                <span
                  className="text-slate dark:text-warm-gray whitespace-nowrap"
